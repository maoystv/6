var rule = {
    ...
    "movie_bt": [
      ...
      },
      {
        ...
        "value": [
          ...
          {
            "n": "动作",
            "v": "/movie_bt_tags/dozuo"
          },
          {
            "n": "动漫",
            "v": "/movie_bt_tags/doman"
          },
          {
            "n": "动画",
            "v": "/movie_bt_tags/dhh"
          ...
          {
            "n": "恐怖",
            "v": "/movie_bt_tags/kubu"
          },
          {
            "n": "悬疑",
            "v": "/movie_bt_tags/xuanyi"
          },
          {
            "n": "情色",
  ...
  class_name: '全部&热映中&动漫&国产剧&美剧&韩剧&日剧&海外剧',
  class_url: 'movie_bt&reyingzhong&/movie_bt_series/dohua&/movie_bt_series/guochanju&/movie_bt_series/mj&/movie_bt_series/hj&/movie_bt_series/rj&/movie_bt_series/hwj',
  play_parse: true,

  /* lazy代码:源于海阔香雅情大佬
  小程序：香情影视 https://pastebin.com/L4tHdvFn8 */
  ...
  //预处理:'rule_fetch_params.headers.Cookie="68148872828e9f4d64e7a296f6c6b6d7=5429da9a54375db451f7f9e4f16ce0ea;esc_search_captcha=1";let new_host="https://www.cz0101.com";let new_html=request(new_host);if(/人机身份验证，请完成以下操作/.test(new_html)){let new_src=pd(new_html,"script&&src",new_host);log(new_src);let hhtml=request(new_src,{withHeaders:true});let json=JSON.parse(hhtml);let html=json.body;let key=html.match(new RegExp(\'var key="(.*?)"\'))[1];let avalue=html.match(new RegExp(\'value="(.*?)"\'))[1];let c="";for(let i=0;i<avalue.length;i++){let a=avalue[i];let b=a.charCodeAt();c+=b}let value=md5(c);log(value);let yz_url="https://www.cz0101.com/a20be899_96a6_40b2_88ba_32f1f75f1552_yanzheng_ip.php?type=96c4e20a0e951f471d32dae103e83881&key="+key+"&value="+value;log(yz_url);hhtml=request(yz_url,{withHeaders:true});json=JSON.parse(hhtml);let setCk=Object.keys(json).find(it=>it.toLowerCase()==="set-cookie");let cookie=setCk?json[setCk].split(";")[0]:"";log("cookie:"+cookie);rule_fetch_params.headers.Cookie=cookie;setItem(RULE_CK,cookie)}'
}
