...
var rule = {
	title: '影探[V2]', // csp_AppYsV2
	 host: 'http://cms.realdou.cn:32653',
  ...
  //hostJs:'print(HOST);let html=request(HOST,{headers:{"User-Agent":"Dart/3.0 (dart:io)"}});let src = JSON.parse(html).domain;print(src);HOST=src',
	...
	quickSearch: 0,
	filterable:1,
	headers:{'User-Agent':'com.github.tvbox.osc.base.App/5.0.4_2 (Linux;Android 12) ExoPlayerLib/2.18.7'},
	timeout:5000,
	class_name:'电影4K&电视剧4K&儿童4K&电影&电视剧&动漫&综艺', // 分类筛选 /api.php/app/nav
	class_url:'6limit&7limit&8limit&1limit&2limit&4limit&3limit',
	play_parse:true,
	lazy:'js:if(/m3u8|mp4/.test(input)){input}else if(/kuying/.test(input)){let purl=request("http://jx.realdou.cn/home/api?type=ys&uid=1308131&key=hnpstuwzAHJNPSY237&url="+input);input={jx:0,url:JSON.parse(purl).url,parse:0}}else if(/qiqi/.test(input)){let purl=request("https://dx.maclsj.com/blue/json.php?url="+input);input={jx:0,url:JSON.parse(purl).url,parse:0}}',
	limit:6,
	推荐:'json:list;vlist;*;*;*;*',
	double: true,
