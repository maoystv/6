...
const getHeaders = (params = {}) => {
    ...
    const sign = sha1(md5(toQueryString({
        ...params,
        key: KEY,
        t
    })));
...
async function home() {
    const [cRes, fRes] = await Promise.all([
        reqSafe(`${currentHost}/api/mw-movie/anonymous/get/filer/type`, {
            headers: getHeaders()
        }),
        reqSafe(`${currentHost}/api/mw-movie/anonymous/v1/get/filer/list`, {
            headers: getHeaders()
        })
    ]);

    const classes = (cRes.data || []).map(k => ({
    ...
    for (const [tid, d] of Object.entries(fData)) {
        const currentSortValues = tid === '1' ? baseSort.slice(1) : baseSort;
        const arr = [{
            key: "type",
            name: "类型",
            value: (d.typeList || []).map(i => ({
                n: i.itemText,
                v: i.itemValue
            }))
        }];
        if (d.plotList?.length) arr.push({
            key: "v_class",
            name: "剧情",
            value: (d.plotList || []).map(i => ({
                n: i.itemText,
                v: i.itemText
            }))
        });
...
async function homeVod() {
    const [r1, r2] = await Promise.all([
        reqSafe(`${currentHost}/api/mw-movie/anonymous/v1/home/all/list`, {
            headers: getHeaders()
        }),
        reqSafe(`${currentHost}/api/mw-movie/anonymous/home/hotSearch`, {
            headers: getHeaders()
        })
    ]);
    let list = [];
    ...
    for (const k in data1)
        if (data1[k]?.list) list.push(...data1[k].list);
    if (Array.isArray(r2.data)) list.push(...r2.data);
    return JSON.stringify({
        list: normalizeVodList(list)
    });
}
...
async function play(_, id) {
    ...
    const urls = (res.data?.list || []).flatMap(item => [item.resolutionName, item.url]);
    return JSON.stringify({
        parse: 0,
        url: urls,
        header: {
